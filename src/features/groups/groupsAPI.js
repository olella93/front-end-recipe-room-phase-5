...
export const fetchGroupRecipesAPI = async (groupId) => {
  const res = await axios.get(`${API_BASE_URL}/groups/${groupId}/recipes`, { headers: authHeader() });
  return res.data;
};

// Join group
export const joinGroupAPI = async (groupId) => {
  const res = await axios.post(`${API_BASE_URL}/groups/${groupId}/join`, {}, { headers: authHeader() });
  return res.data;
};

// Leave group
export const leaveGroupAPI = async (groupId) => {
  const res = await axios.delete(`${API_BASE_URL}/groups/${groupId}/leave`, { headers: authHeader() });
  return res.data;
};
